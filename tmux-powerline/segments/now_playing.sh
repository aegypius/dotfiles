...
__np_spotify() {
	if shell_is_linux; then
		...
		if [ "$?" -eq 0 ] && [ -n "$metadata" ]; then
			...
			# state=$(dbus-send --reply-timeout=42 --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:'org.mpris.MediaPlayer2.Player' string:'PlaybackStatus')
			state="Playing"
